...
//!	Generated Date	: Thu, 13, Jul 2023  
	File Path	: DefaultComponent\Garbage_Bin_Simulation\SGCS.cpp
*********************************************************************/
...
//#[ ignore
#define BDD_SGCS_SGCS_SERIALIZE OM_NO_OP
//#]
...
SGCS::SGCS(void) : itsCommunication_System(NULL), itsElectricity_provider(NULL), itsGarbage_Disposal_Sites(NULL), itsMain_Server(NULL), itsRecycling_center(NULL), itsTemperature_sensor(NULL), itsTruck(NULL), itsTruck_refuel_stations(NULL), itsUsers(NULL), itsVolume_sensor(NULL), itsWeight_sensor(NULL) {
    NOTIFY_CONSTRUCTOR(SGCS, SGCS(), 0, BDD_SGCS_SGCS_SERIALIZE);
}
...
const Software_Controllers* SGCS::getItsSoftware_Controllers(void) const {
...
void SGCS::cleanUpRelations(void) {
    ...
    if(itsElectricity_provider != NULL)
        {
            NOTIFY_RELATION_CLEARED("itsElectricity_provider");
    ...
    if(itsGarbage_Disposal_Sites != NULL)
        {
            NOTIFY_RELATION_CLEARED("itsGarbage_Disposal_Sites");
    ...
    if(itsRecycling_center != NULL)
        {
            NOTIFY_RELATION_CLEARED("itsRecycling_center");
    ...
    if(itsTruck_refuel_stations != NULL)
        {
            NOTIFY_RELATION_CLEARED("itsTruck_refuel_stations");
    ...
    if(itsUsers != NULL)
        {
            NOTIFY_RELATION_CLEARED("itsUsers");
    ...
    if(itsVolume_sensor != NULL)
        {
            NOTIFY_RELATION_CLEARED("itsVolume_sensor");
    ...
    if(itsWeight_sensor != NULL)
        {
            NOTIFY_RELATION_CLEARED("itsWeight_sensor");
...
void SGCS::__setItsElectricity_provider(Electricity_provider* const p_Electricity_provider) {
    ...
    if(p_Electricity_provider != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsElectricity_provider", p_Electricity_provider, false, true);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsElectricity_provider");
...
void SGCS::_addItsGarbage_Bin(Garbage_Bin* const p_Garbage_Bin) {
    if(p_Garbage_Bin != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsGarbage_Bin", p_Garbage_Bin, false, false);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsGarbage_Bin");
        }
    itsGarbage_Bin.add(p_Garbage_Bin);
}

void SGCS::_removeItsGarbage_Bin(Garbage_Bin* const p_Garbage_Bin) {
    NOTIFY_RELATION_ITEM_REMOVED("itsGarbage_Bin", p_Garbage_Bin);
...
void SGCS::__setItsGarbage_Disposal_Sites(Garbage_Disposal_Sites* const p_Garbage_Disposal_Sites) {
    ...
    if(p_Garbage_Disposal_Sites != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsGarbage_Disposal_Sites", p_Garbage_Disposal_Sites, false, true);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsGarbage_Disposal_Sites");
...
void SGCS::__setItsRecycling_center(Recycling_center* const p_Recycling_center) {
    ...
    if(p_Recycling_center != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsRecycling_center", p_Recycling_center, false, true);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsRecycling_center");
...
void SGCS::__setItsTruck_refuel_stations(Truck_refuel_stations* const p_Truck_refuel_stations) {
    ...
    if(p_Truck_refuel_stations != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsTruck_refuel_stations", p_Truck_refuel_stations, false, true);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsTruck_refuel_stations");
...
void SGCS::__setItsUsers(Users* const p_Users) {
    ...
    if(p_Users != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsUsers", p_Users, false, true);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsUsers");
...
void SGCS::__setItsVolume_sensor(Volume_sensor* const p_Volume_sensor) {
    ...
    if(p_Volume_sensor != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsVolume_sensor", p_Volume_sensor, false, true);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsVolume_sensor");
...
void SGCS::__setItsWeight_sensor(Weight_sensor* const p_Weight_sensor) {
    ...
    if(p_Weight_sensor != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsWeight_sensor", p_Weight_sensor, false, true);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsWeight_sensor");
...
void OMAnimatedSGCS::serializeRelations(AOMSRelations* aomsRelations) const {
    aomsRelations->addRelation("itsRecycling_center", false, true);
    if(myReal->itsRecycling_center)
        {
            aomsRelations->ADD_ITEM(myReal->itsRecycling_center);
        }
    aomsRelations->addRelation("itsGarbage_Disposal_Sites", false, true);
    if(myReal->itsGarbage_Disposal_Sites)
        {
            aomsRelations->ADD_ITEM(myReal->itsGarbage_Disposal_Sites);
        }
    aomsRelations->addRelation("itsTruck_refuel_stations", false, true);
    if(myReal->itsTruck_refuel_stations)
        {
            aomsRelations->ADD_ITEM(myReal->itsTruck_refuel_stations);
        }
    aomsRelations->addRelation("itsElectricity_provider", false, true);
    if(myReal->itsElectricity_provider)
        {
            aomsRelations->ADD_ITEM(myReal->itsElectricity_provider);
        }
    aomsRelations->addRelation("itsUsers", false, true);
    if(myReal->itsUsers)
        {
            aomsRelations->ADD_ITEM(myReal->itsUsers);
        }
    ...
    aomsRelations->addRelation("itsWeight_sensor", false, true);
    if(myReal->itsWeight_sensor)
        {
            aomsRelations->ADD_ITEM(myReal->itsWeight_sensor);
        }
    aomsRelations->addRelation("itsVolume_sensor", false, true);
    if(myReal->itsVolume_sensor)
        {
            aomsRelations->ADD_ITEM(myReal->itsVolume_sensor);
        }
    aomsRelations->addRelation("itsGarbage_Bin", false, false);
    {
        OMIterator<Garbage_Bin*> iter(myReal->itsGarbage_Bin);
        while (*iter){
    ...
    aomsRelations->addRelation("itsSoftware_Controllers", true, true);
    aomsRelations->ADD_ITEM(&myReal->itsSoftware_Controllers);
}
//#]

IMPLEMENT_META_P(SGCS, BDD, BDD, false, OMAnimatedSGCS)
#endif // _OMINSTRUMENT
