...
//!	Generated Date	: Wed, 12, Jul 2023  
	File Path	: DefaultComponent\Garbage_Bin_Simulation\Communication_System.cpp
*********************************************************************/
...
#include "SGCS.h"
//#[ ignore
#define BDD_Communication_System_Communication_System_SERIALIZE OM_NO_OP
//#]
...
Communication_System::Communication_System(void) : itsData_Storage(NULL), itsDispatch(NULL), itsGarbage_Bin(NULL), itsMain_Server(NULL), itsSGCS(NULL) {
    NOTIFY_CONSTRUCTOR(Communication_System, Communication_System(), 0, BDD_Communication_System_Communication_System_SERIALIZE);
}
...
void Communication_System::__setItsGarbage_Bin(Garbage_Bin* const p_Garbage_Bin) {
    ...
    if(p_Garbage_Bin != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsGarbage_Bin", p_Garbage_Bin, false, true);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsGarbage_Bin");
...
void Communication_System::_addItsGarbage_Bin_1(Garbage_Bin* const p_Garbage_Bin) {
    if(p_Garbage_Bin != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsGarbage_Bin_1", p_Garbage_Bin, false, false);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsGarbage_Bin_1");
        }
    itsGarbage_Bin_1.add(p_Garbage_Bin);
}

void Communication_System::_removeItsGarbage_Bin_1(Garbage_Bin* const p_Garbage_Bin) {
    NOTIFY_RELATION_ITEM_REMOVED("itsGarbage_Bin_1", p_Garbage_Bin);
...
void Communication_System::_addItsGarbage_Bin_2(Garbage_Bin* const p_Garbage_Bin) {
    if(p_Garbage_Bin != NULL)
        {
            NOTIFY_RELATION_ITEM_ADDED("itsGarbage_Bin_2", p_Garbage_Bin, false, false);
        }
    else
        {
            NOTIFY_RELATION_CLEARED("itsGarbage_Bin_2");
        }
    itsGarbage_Bin_2.add(p_Garbage_Bin);
}

void Communication_System::_removeItsGarbage_Bin_2(Garbage_Bin* const p_Garbage_Bin) {
    NOTIFY_RELATION_ITEM_REMOVED("itsGarbage_Bin_2", p_Garbage_Bin);
...
void OMAnimatedCommunication_System::serializeRelations(AOMSRelations* aomsRelations) const {
    ...
    aomsRelations->addRelation("itsGarbage_Bin", false, true);
    if(myReal->itsGarbage_Bin)
        {
            aomsRelations->ADD_ITEM(myReal->itsGarbage_Bin);
        }
    aomsRelations->addRelation("itsGarbage_Bin_1", false, false);
    {
        OMIterator<Garbage_Bin*> iter(myReal->itsGarbage_Bin_1);
        while (*iter){
            aomsRelations->ADD_ITEM(*iter);
            iter++;
        }
    }
    aomsRelations->addRelation("itsGarbage_Bin_2", false, false);
    {
        OMIterator<Garbage_Bin*> iter(myReal->itsGarbage_Bin_2);
        while (*iter){
            aomsRelations->ADD_ITEM(*iter);
            iter++;
        }
...
IMPLEMENT_META_P(Communication_System, BDD, BDD, false, OMAnimatedCommunication_System)
#endif // _OMINSTRUMENT
